...
(() => {
  ...
  function injectUI(){
    ...
    root.innerHTML=`<div class="nya-overlay" id="nyaOverlay" hidden>
      <section class="nya-panel" role="dialog" aria-modal="true" aria-labelledby="nyaTitle">
        <button class="nya-lang-toggle notranslate" id="nyaLangToggle" type="button" aria-label="English">EN</button>
    ...
    const langBtn=byId('nyaLangToggle');if(langBtn){const l=window.NineYinI18n?.getLanguage?.()||'ru';langBtn.textContent=l==='en'?'RU':'EN';langBtn.setAttribute('aria-label',l==='en'?'Русский':'English');langBtn.addEventListener('click',()=>window.NineYinI18n?.setLanguage?.(l==='en'?'ru':'en'));}
  ...
  window.NineYinAccount={version:'0.0.29',ready,access,getCurrent,refreshProfile,listProfiles,saveOwnProfile,createPost,getPost,listPosts,listCategories,addCategory,removeCategory,setPostPinned,getPrivateItemCodes,isAdmin,setUserBlocked,logout,openAuth,defaultAvatar,isDemo:()=>demo};
